...
For every document in data/all_documents.json this script:
  1. asks the local Qwen model for structured metadata
     (summary / component / error_type / tags),
...
Usage:
    ...
    uv run python data_pipeline/enrich_documents.py --workers 8 --limit 50
    uv run python data_pipeline/enrich_documents.py --rebuild-npy      # only re-export the .npy
"""
...
def build_user_message(doc: dict) -> str:
    # Minimal schema: id, title, category, problem, resolution.
    # `problem` already carries source + symptom + root cause merged in;
    # `resolution` already carries the fix + key learning + config diff.
    parts = [f"Title: {doc.get('title', '')}"]
    if doc.get("problem"):
        parts.append(f"Problem: {doc['problem'][:3500]}")
    if doc.get("resolution"):
        parts.append(f"Resolution: {doc['resolution'][:2500]}")
    return "\n\n".join(parts)


def build_doc_text(doc: dict) -> str:
    """Synthesize the searchable/embeddable blob at enrichment time —
    not stored in the source scenario JSON, computed here instead."""
    parts = []
    if doc.get("title"):
        parts.append(f"Title: {doc['title']}")
    if doc.get("problem"):
        parts.append(f"Problem: {doc['problem']}")
    if doc.get("resolution"):
        parts.append(f"Resolution: {doc['resolution']}")
    return "\n\n".join(parts)[:5000]
...
METADATA_SCHEMA = {
    ...
    "required": ["summary", "component", "error_type", "tags"],
}
...
def embed_text(doc: dict) -> list[float] | None:
    text = doc.get("text") or build_doc_text(doc)
...
def process_doc(doc: dict) -> dict:
    ...
    # Synthesize the searchable blob now — source JSON doesn't store one.
    enriched["text"] = build_doc_text(doc)
...
def main() -> int:
    ...
    for field in ("component", "error_type"):
        counts: dict[str, int] = {}
        for doc in enriched:
